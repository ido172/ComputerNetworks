...
public class HttpResponseMaker {
	...
	public HttpResponse handlePOSTRequest() throws IOException {
		...
		if (user == null) {
			if (location.equals("main.html") && params.containsKey("login")) { // location is "/" no
				// cookie
				response = HttpResponse.RESPONSE_200_OK;
			...
			if (handler.isDeleteRequest()  && handler.isValidDeleteRequest(user)) {
				handler.deleteReminderInDateBase(Integer.parseInt(params.get(DataXMLManager.ID)));
				location = "reminders.html";
				response = HttpResponse.RESPONSE_302_REDIRECT;
			} else if (handler.isEditRequest() && handler.isValidReminderEdit(user)) {
				result = handler.editReminderInDateBase(user, Integer.parseInt(params.get(DataXMLManager.ID)));
				location = "reminders.html";
				response = HttpResponse.RESPONSE_302_REDIRECT;
			} else if (handler.isValidNewReminder()) {
				result = handler.createNewReminderInDataBase(user);
				location = "reminders.html";
				response = HttpResponse.RESPONSE_302_REDIRECT;
			} else {
			...
			boolean result = true;
			if (handler.isDeleteRequest() && handler.isValidDeleteRequest(user)) {
				handler.deleteTaskInDateBase(Integer.parseInt(params.get(DataXMLManager.ID)));
				location = "tasks.html";
				response = HttpResponse.RESPONSE_302_REDIRECT;
			} else if (handler.isValidNewTask()) {
				result = handler.createNewTaskInDataBase(user);
				location = "tasks.html";
				response = HttpResponse.RESPONSE_302_REDIRECT;
			} else {
				response = HttpResponse.RESPONSE_200_OK;	//Error page
