...
final class HttpRequestHandler implements Runnable {
	...
	private void sendResponseToClient(HttpResponse httpResponse) throws IOException {
	...
	private HttpResponse processRequest() {
		try {
			...
			if (parsedHttpRequest == null) {
				return httpResponseMaker.makeErrorPageResponse(HttpResponse.RESPONSE_400_BAD_REQUEST,
						"Your browser sent a request that this server did not understand");
			}

			/*
			 * Check if head request (this check is here because even if response would be an error response, we will
			...
			// Validate HTTP Protocol
				...
				return httpResponseMaker.makeErrorPageResponse(HttpResponse.RESPONSE_505_BAD_HTTP_VERSION,
			...
			return httpResponseMaker.makeErrorPageResponse(HttpResponse.RESPONSE_408_REQUEST_TIMEOUT,
					"The server timed out while waiting for the browser's request");
		} catch (Exception e) {
			e.printStackTrace();
			return httpResponseMaker.makeErrorPageResponse(HttpResponse.RESPONSE_500_INTERNAL_ERROR, "Internal Error");
	...
	private HttpResponse processHttpResponse() throws IOException {
		...
		// If non of the above condition passed than method is not implemented
			...
			result = httpResponseMaker.makeErrorPageResponse(HttpResponse.RESPONSE_501_NOT_IMPLEMENTED,
	...
	private boolean checkKeepalive() {
		return (parsedHttpRequest.get(RequestParser.PROTOCOL).equals(PROTOCOL_HTTP11)
				&& parsedHttpRequest.containsKey(HttpHeaders.HEADER_CONNECTION.toLowerCase()) && parsedHttpRequest.get(
				HttpHeaders.HEADER_CONNECTION.toLowerCase()).equals(HttpHeaders.CONNECTION_KEEP_ALIVE.toLowerCase()));
	}

	private boolean checkChunked() {
		return (parsedHttpRequest.containsKey(HttpHeaders.HEADER_CHUNKED.toLowerCase()) && parsedHttpRequest.get(
				HttpHeaders.HEADER_CHUNKED.toLowerCase()).equals(HttpHeaders.CHUNKED_YES.toLowerCase()));
	}
	...
	private boolean isValidProtocol(String protocol) {
		return (protocol.equals(PROTOCOL_HTTP11) || protocol.equals(PROTOCOL_HTTP10));
	}

	private boolean isValidLocation(String location) {
		// Validate location does not contain ".." spaces and start with "/"
		return !(location.contains("..") || location.contains(" ") || !(location.charAt(0) == '/'));
	}
